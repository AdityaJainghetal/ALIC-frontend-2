...
export const createChoose = createAsyncThunk(
  ...
  async (formData, { rejectWithValue }) => {
    try {
      const response = await axios.post('https://alic-backend-2.onrender.com/choose/create', formData, {
        headers: {
...
export const fetchChooseItems = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const res = await axios.get(`https://alic-backend-2.onrender.com/choose/display`);
      return res.data;
    } catch (err) {
...
export const deleteChooseItem = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      await axios.delete(`https://alic-backend-2.onrender.com/choose/${id}`);
      return id;
    } catch (err) {
      return rejectWithValue(err.response?.data?.message || err.message);
    }
