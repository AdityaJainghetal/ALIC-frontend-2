...
const Judgement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to create judgement');
    } finally {
      setLoading(false);
    }
  };
    ...
    if (field.name === 'description') {
      return (
        <div key={field.name} className="space-y-1 md:col-span-2">
          ...
          <div className={`border ${errors[field.name] ? 'border-red-500' : 'border-gray-300'} rounded-md`}>
            <CKEditor
    ...
    return (
      <div key={field.name} className="space-y-1">
        ...
        <input
          ...
          onChange={handleInputChange}
          className={`w-full p-2 border ${errors[field.name] ? 'border-red-500' : 'border-gray-300'} rounded-md`}
        />
        {errors[field.name] && <p className="text-red-500 text-xs mt-1">{errors[field.name]}</p>}
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <form onSubmit={handleSubmit} className="w-full max-w-5xl mx-auto p-8 bg-white rounded-lg shadow-md">
        <ToastContainer position="top-center" autoClose={3000} />
        <h2 className="text-2xl font-bold mb-6 text-center text-gray-800">Judgement Registration</h2>
