...
const EnrollDisplay = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      const response = await fetch('https://alic-backend-2.onrender.com/enroll/alldisplay');
      if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://alic-backend-2.onrender.com/enroll/alldelete/${id}`, {
        method: 'DELETE',
      });
