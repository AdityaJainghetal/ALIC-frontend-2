...
const EnquiryDisplay = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      const response = await fetch('https://alic-backend-2.onrender.com/enquiry/allcourse');
      if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://alic-backend-2.onrender.com/enquiry/coursedelte/${id}`, {
        method: 'DELETE',
      });
