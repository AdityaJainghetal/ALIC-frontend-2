...
const WhatsNewDisplay = () => {
  ...
  const fetchWhatsNew = async () => {
    try {
      const response = await fetch('https://alic-backend-2.onrender.com/whatsnew/alldisplay');
      if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://alic-backend-2.onrender.com/whatsnew/delete/${id}`, {
        method: 'DELETE',
      });
