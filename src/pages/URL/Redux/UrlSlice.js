...
export const addUrl = createAsyncThunk('urls/addUrl', async (urlData) => {
  const response = await axios.post('https://alic-backend-2.onrender.com/url/create', urlData);
...
export const fetchAllUrls = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get('https://alic-backend-2.onrender.com/url/display');
...
export const fetchUrlById = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      const response = await axios.get(`https://alic-backend-2.onrender.com/url/${id}`);
      return response.data.data;
    } catch (error) {
      return rejectWithValue(error.response?.data || error.message);
    }
...
export const deleteUrl = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      await axios.delete(`https://alic-backend-2.onrender.com/url/${id}`);
      return id;
    } catch (error) {
      return rejectWithValue(error.response?.data || error.message);
    }
